...
def figure1() -> dict[str, float]:
    ...
    for i, (wl, label) in enumerate(zip(wls_probe, labels, strict=True)):
        ...
        modes = solve_modes(coupled_structures(), wl, mesh=mesh, num_modes=2)
        mode = modes[0]
    ...
    n_b = np.array([fundamental_neff(wgb_structures(), wl, mesh=mesh) for wl in wls])
    ax_e.plot(wls * 1e3, n_b, "k--", label="WGB (3 segments)")
    cutoffs: dict[str, float] = {}
    for w_a in widths_a:
        n_a = np.array(
            [fundamental_neff(wga_structures(w_a), wl, mesh=mesh) for wl in wls]
        )
    ...
    n_plus, n_minus, n_as, n_bs = [], [], [], []
    for wl in wls_f:
        modes = solve_modes(coupled_structures(), wl, mesh=mesh, num_modes=2)
        te = [m for m in modes if m.te_fraction > 0.5][:2]
        n_plus.append(float(np.real(te[0].neff)))
        n_minus.append(float(np.real(te[1].neff)))
        n_as.append(fundamental_neff(wga_structures(W_A), wl, mesh=mesh))
        n_bs.append(fundamental_neff(wgb_structures(), wl, mesh=mesh))
...
def main() -> dict[str, object]:
    ...
    return {"cutoffs_um": cutoffs, **fig2, **fig3, **fig4}
